...
O argumento `stringsAsFactors = F` evita que o R abra o seu banco lendo as strings (textos) como se fossem da classe `factor`. A classe `factor` é apropriada para se trabalhar com variáveis categóricas. Contudo, elas trazem um elemento adicional de complexidade, desnecessária para as análises que conduzimos aqui. Sendo assim, o ideal é que todas as nossas variáveis de texto sejam lidas como `character`.
...
```{r var_1, eval=FALSE}
var(banco$mortalidade_91)
sd(banco$mortalidade_91)

var(banco$mortalidade_00)
sd(banco$mortalidade_00)

var(banco$mortalidade_10)
sd(banco$mortalidade_10)
```
